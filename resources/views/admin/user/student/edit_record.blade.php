...
@section('page_description', $student->name . ' ( '. $student->university_id . ' )')
@section('content')
<div class="content">
      ...
      <form action="{{ route('admin.student.record.update',
        ...
        <div class="col-md-6">
          <div class="form-group{{ $errors->has('funding_type_id') ? ' has-error' : '' }}">
            ...
            <select class="form-control" name="funding_type_id">
              ...
              <option value="{{ $t->id }}"
                @if($errors->any()) {{ (collect(old('funding_type_id'))->contains($t->id)) ? 'selected':'' }}/>
                @else {{ ($record->fundingType->id == $t->id) ? 'selected':'' }}/> @endif
                {{ $t->name }}
              </option>
              @endforeach
            </select>
        ...
        <div class="col-md-6">
          <div class="form-group{{ $errors->has('enrolment_date') ? ' has-error' : '' }}">
            ...
            name="enrolment_date"
            id="enrolment_date"
            value="{{ old('enrolment_date') ?? $record->enrolment_date->format('Y-m-d') }}">
          ...
          <div class="form-group{{ $errors->has('tierFour') ? ' has-error' : '' }}">
            ...
            <label class="radio-inline">
              ...
              @else {{ $record->tierFour === 1 ? 'checked="checked"' : '' }}> @endif
              Yes
            </label>
            <label class="radio-inline">
              <input type="radio" name="tierFour" value="0" @if($errors->any()){{ old('tierFour') === 0 ? 'checked="checked"' : '' }}>
              @else {{ $record->tierFour === 0 ? 'checked="checked"' : '' }}> @endif No
            </label>
